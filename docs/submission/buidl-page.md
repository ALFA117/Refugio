...
**Retention & Discovery Value.** Persistent embers and an **in-world leaderboard** (top guardians, always visible) give players a reason to return; the referral loop turns each guardian into a discovery channel. A graceful "lighting the fire…" state covers the server's cold-start so newcomers never hit a broken-looking scene. *(Optional stretch: a public companion leaderboard site so the ranking is shareable outside Decentraland.)*
